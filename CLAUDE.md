...
Console routing (0.12.0): INFO and below → stdout (plain message, reads like
`print()`); WARNING+ → stderr with level prefix. Same in CLI and agent mode —
in agent mode `runspec serve` captures stdout as the MCP tool response, so
`logger.info` lines reach the agent automatically. File handler (JSON, DEBUG)
is always on as the audit trail.
...
Console routing (0.11.0) mirrors Python: INFO and below → stdout; WARNING+ → stderr with level prefix.
